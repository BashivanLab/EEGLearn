...
def gen_images(locs, features, n_gridpoints, normalize=True,
    ...
    Parameters:
    -----------
    - locs: An array with shape [n_electrodes, 2] containing X, Y
...
def build_cnn(input_var=None, w_init=None, n_layers=(4, 2, 1),
              n_filters_first=32, imsize=32, n_colors=3):
    """build_cnn : builds a VGG style CNN network followed by a
    fully-connected layer and a softmax layer. Stacks are separated
    by a maxpool layer. Number of kernels in each layer is twice
    ...
    - input_var: theano variable as input to the network
    - w_init: Initial weight values
    - n_layers: number of layers in each stack. An array of integers with each
        value corresponding to the number of layers in each stack.
        (e.g. [4, 2, 1] == 3 stacks with 4, 2, and 1 layers in each.
    - n_filters_first: number of filters in the first layer
    - imSize: Size of the image
    - n_colors: Number of color channels (depth)
...
def build_convpool_max(input_vars, nb_classes, imsize=32,
                       n_colors=3, n_timewin=3):
    """build_convpool_max: builds the complete network with maxpooling
    layer in time.

    Parameters:
    -----------
    - input_vars: list of EEG images (one image per time window)
    - nb_classes: number of classes
    - imsize: size of the input image (assumes a square input)
    - n_colors: number of color channels in the image
    - n_timewin: number of time windows in the snippet

    Returns:
    --------
    - a pointer to the output of last layer
    """
    ...
    for i in range(n_timewin):
        if i == 0:
            convnet, w_init = build_cnn(
                input_vars[i], imsize=imsize, n_colors=n_colors)
        else:
            convnet, _ = build_cnn(
                input_vars[i], w_init=w_init, imsize=imsize, n_colors=n_colors)
...
def build_convpool_conv1d(input_vars, nb_classes,
                          imsize=32, n_colors=3, n_timewin=3):
    """build_convpool_conv1d: builds the complete network with 1D-conv layer
    ...
    # input to 1D convlayer should be in
...
def build_convpool_lstm(input_vars, nb_classes,
                        grad_clip=110, imsize=32, n_colors=3, n_timewin=3):
    """build_convpool_lstm: builds the complete network with LSTM layer to
    integrate time from sequences of EEG images.

    Parameters:
    -----------
    - input_vars: list of EEG images (one image per time window)
    - nb_classes: number of classes
    - grad_clip:  the gradient messages are clipped to the given value during
        the backward pass.
    - imsize: size of the input image (assumes a square input)
    - n_colors: number of color channels in the image
    - n_timewin: number of time windows in the snippet
    ...
    # Input to LSTM should have the shape as (batch size,
...
def build_convpool_mix(input_vars, nb_classes,
                       grad_clip=110, imsize=32, n_colors=3, n_timewin=3):
    """build_convpool_mix: builds the complete network with LSTM and 1D-conv
    layers combined
    ...
    # Input to LSTM should have the shape as
    ...
    # A fully-connected layer of 256 units with 50% dropout on its inputs:
    convpool = DenseLayer(
        lasagne.layers.dropout(dense_input, p=.5),
        num_units=512,
        nonlinearity=lasagne.nonlinearities.rectify
    )
...
def iterate_minibatches(inputs, targets, batchsize, shuffle=False):
    """iterate_minibatches: iterates over the samples returing batches
    of size batchsize.

    Parameters:
    -----------
    - inputs: input data array. It should be a 4D numpy array for images
        [n_samples, n_colors, W, H] and 5D numpy array if working with
        sequence of images [n_timewindows, n_samples, n_colors, W, H].
    - targets: vector of target labels.
    - batchsize: Batch size
    - shuffle: Flag whether to shuffle the samples before iterating or not.

    Returns:
    --------
    - images and labels for a batch
    """
...
def train(images, labels, fold, model_type, batch_size=32, num_epochs=5):
    """train: a sample training function which loops over the training set
    and evaluates the network on the validation set after each epoch.

    Parameters:
    -----------
    - images: input images
    - labels: target labels
    - fold: tuple of (train, test) index numbers
    - model_type: model type ('cnn', '1dconv', 'maxpool', 'lstm', 'mix')
    - batch_size: batch size for training
    - num_epochs: number of epochs of dataset to go over for training
...
if __name__ == '__main__':
    ...
    feats = scipy.io.loadmat(
        '../Sample data/FeatureMat_timeWin.mat')['features']
    subj_nums = np.squeeze(scipy.io.loadmat(
        '../Sample data/trials_subNums.mat')['subjectNum'])
    ...
    av_feats = reduce(
        lambda x, y: x + y,
        [feats[:, i * 192:(i + 1) * 192] for i in range(feats.shape[1] / 192)]
    )
