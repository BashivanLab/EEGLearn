...
def augment_EEG(data, stdMult, pca=False, n_components=2):
    """augment_EEG: augment data by adding normal noise to each feature.

    Parameters:
    -----------
    - data: EEG feature data as a matrix (n_samples x n_features)
    - stdMult: Multiplier for std of added noise
    - pca: if True will perform PCA on data and add noise
        proportional to PCA components.
    - n_components: Number of components to consider when using PCA.

    Returns:
    --------
    - Augmented data as a matrix (n_samples x n_features)
    """
    ...
    if pca:
        ...
        # Add Gaussian noise with std determined by weighted std
        # of each feature
...
def augment_EEG_image(image, std_mult, pca=False, n_components=2):
    """augment_EEG_image: augment data by adding normal noise to each feature.

    Parameters:
    -----------
    - image: EEG feature data as a a colored image [n_samples, n_colors, W, H]
    - std_mult: Multiplier for std of added noise
    - pca: if True will perform PCA on data and add noise proportional to PCA
        components.
    - n_components: Number of components to consider when using PCA.
    ...
    for c in xrange(image.shape[1]):
        reshData = np.reshape(
            data['featMat'][:, c, :, :], (data['featMat'].shape[0], -1))

        if pca:
            augData[:, c, :] = augment_EEG(
                reshData, std_mult, pca=True, n_components=n_components)
...
def load_data(data_file):
    """load_data: loads the data from MAT file. MAT file should contain two
...
def reformatInput(data, labels, indices):
    """reformatInput: receives the the indices for train and test datasets.
